...
function serialize(b: { id: string; startDate: Date; endDate: Date; label: string }) {
  return {
    id: b.id,
...
export async function GET() {
  ...
  const entries = await prisma.blockedDate.findMany({ where: { clientId }, orderBy: { startDate: "asc" } });
...
export async function PATCH(req: NextRequest) {
  ...
  const { id, startDate, endDate, label } = await req.json();
